...
import { generateSeed, generatePassphrase, emptyByte } from '../../logic/wallets';
      ...
      <div>
        <div className="columns">
          <div className="column is-8 is-offset-2">
            {isTouch ?
              <div>
                <p className="title-seed-message">Enter text below to generate random bytes</p>
                <input
                  className="input message-input"
                  type="text"
                  placeholder="Enter random text"
                  onChange={event => this.seedGeneratorBoundToThis(event.target.value)}
                />
              </div> :
              <p className="title-seed-message">Move your mouse to generate random bytes</p>
            }
          </div>
        </div>
        <div className="columns">
          <div className="column is-8 is-offset-2">
            Progress: {this.state.data ? parseFloat(this.state.data.percentage).toFixed(2) : 0} %
          </div>
        </div>
        <div className="columns">
          <div className="column is-8 is-offset-2">
